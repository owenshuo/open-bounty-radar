...
The Pages dashboard workflow runs on pushes and manual dispatch. A dedicated Live Dashboard Refresh workflow runs on manual dispatch and a staggered daily schedule at 01:17 UTC. GitHub Actions supports scheduled workflows at a minimum interval of five minutes, but scheduled runs can be delayed during high load, so the default avoids exact hour and half-hour boundaries and keeps Pages deployment notifications quieter.
